...
with serial.Serial(PORT, BAUD, timeout=0.1) as voice:
    ...
    print(
        f"Training record {RECORD_NUMBER} "
        f"as another version of {COMMAND_NAME}"
    )
    print(
        "Keep the microphone still and be ready "
    ...
    try:
        while time.time() < end_time:
            ...
            if "speak now" in lower_text:
                ...
                print(
                    "Wait for the next Speak now prompt.\n"
                )

            elif "too noisy" in lower_text:
                print("\nToo noisy.")
                print(
                    "Keep the microphone still and move "
                    "to a quieter place.\n"
                )
